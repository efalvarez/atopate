        ...
        if (callback != null) {
            ...
            vistaMapa.getMapAsync(new OnMapReadyCallback() {
                ...
                public void onMapReady(GoogleMap mapa) {
                    ...
                    mapa.addPolyline(new PolylineOptions().add(madrid, fic).color(Color.BLUE));

                    // Polyline center
                    LatLng center = new LatLngBounds.Builder().include(fic).include(madrid).build().getCenter();

                    // For zooming automatically to the location of the marker
                    CameraPosition cameraPosition = new CameraPosition.Builder().target(center).zoom(5).build();
